...
metadata:
  skillforge:
    ...
    globs:
      - "**/*"
    attribution: >-
      Adapted from addyosmani/agent-skills (MIT), skills/performance-optimization
      at df1edb2e0548.
    params:
      test_command:
        type: string
      ...
      bundle_budget_kb:
        type: int
        description: Initial-load JavaScript budget in KB, gzipped.
        default: 200
      layers:
        type: list
        items: [frontend, backend, database]
        description: Which layers this project actually owns.
...
<!-- skillforge:block id=antipattern-n1-queries when='"database" in params.layers' -->
#### N+1 Queries (Backend)
...
<!-- skillforge:block id=antipattern-images when='"frontend" in params.layers' -->
...
<!-- skillforge:block id=antipattern-rerenders when='"frontend" in params.layers' -->
...
<!-- skillforge:block id=antipattern-bundle-size when='"frontend" in params.layers' -->
#### Large Bundle Size
...
<!-- skillforge:block id=antipattern-caching when='"backend" in params.layers' -->
...
```
JavaScript bundle: < {{ params.bundle_budget_kb }}KB gzipped (initial load)
CSS: < 50KB gzipped
...
<!-- skillforge:block id=verification -->
## Verification
...
- [ ] `{{ params.test_command }}` still passes (optimization didn't break behavior)
